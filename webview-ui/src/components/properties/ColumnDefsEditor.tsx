...
function extractToken(className: string, pattern: RegExp): string | undefined {
  return className.split(/\s+/).filter(Boolean).find((c) => pattern.test(c));
}

function replaceToken(className: string, oldToken: string | undefined, newToken: string | undefined): string {
  let classes = className.split(/\s+/).filter(Boolean);
  if (oldToken) classes = classes.filter((c) => c !== oldToken);
...
function TailwindColorPicker({ prefix, label, value, onChange }: TailwindColorPickerProps) {
  ...
  const blackVal = `${prefix}-black`;
  const whiteVal = `${prefix}-white`;
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <div className="flex items-center gap-1">
        ...
        <button
          type="button"
          onClick={() => onChange(currentValue === whiteVal ? undefined : whiteVal)}
          title="white"
          className={`h-3.5 w-3.5 rounded-sm border border-[var(--vscode-input-border,#555)] transition-all ${
            currentValue === whiteVal
              ? "ring-2 ring-[var(--vscode-focusBorder,#007fd4)] ring-offset-1 ring-offset-[var(--vscode-editor-background,#1e1e1e)]"
              : "hover:scale-110"
          }`}
...
function StylePanel({ value, onChange, onClose }: StylePanelProps) {
  const cls = value ?? "";

  function updateColor(pattern: RegExp, newToken: string | undefined) {
    const old = extractToken(cls, pattern);
    const next = replaceToken(cls, old, newToken);
    onChange(next || undefined);
  }

  function updateToken(pattern: RegExp, newToken: string | undefined) {
    const old = extractToken(cls, pattern);
    const next = replaceToken(cls, old, newToken);
    onChange(next || undefined);
  }

  const bgToken = extractToken(cls, /^bg-/);
  const textToken = extractToken(cls, /^text-[a-z]+-\d{2,3}$|^text-(black|white)$/);
  const hoverBgToken = extractToken(cls, /^hover:bg-/);
  const borderColorToken = extractToken(cls, /^border-[a-z]+-\d{2,3}$|^border-(black|white)$/);
  const borderWToken = extractToken(cls, /^border(-[248])?$/);
  const radiusToken = extractToken(cls, /^rounded/);
  const fontSizeToken = extractToken(cls, /^text-(xs|sm|base|lg|xl)$/);
  const fontFamilyToken = extractToken(cls, /^font-/);
  const textAlignToken = extractToken(cls, /^text-(left|center|right)$/);
  const vAlignToken = extractToken(cls, /^items-/);
  const shadowToken = extractToken(cls, /^shadow/);
  ...
  return (
    ...
    >
      <div
        className="fixed left-1/2 top-1/2 z-50 max-h-[85vh] w-64 -translate-x-1/2 -translate-y-1/2 overflow-y-auto rounded border border-[var(--vscode-panel-border,#444)] bg-[var(--vscode-editor-background,#1e1e1e)] p-3 shadow-xl"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="mb-2 flex items-center justify-between">
          <span className="text-xs text-[var(--vscode-foreground,#ccc)]">Button Style</span>
          <button
            type="button"
            onClick={onClose}
            className={`${BTN_CLASS} px-1.5 py-0`}
          >
            ✕
          </button>
        </div>
        ...
        <div className="flex flex-col gap-3">
          ...
          <TailwindColorPicker
            ...
            label="text color"
            value={textToken}
            onChange={(v) => updateColor(/^text-[a-z]+-\d{2,3}$|^text-(black|white)$/, v)}
          />
          <TailwindColorPicker
            prefix="hover:bg"
            label="hover bg"
            value={hoverBgToken}
            onChange={(v) => updateColor(/^hover:bg-/, v)}
          />
          <TailwindColorPicker
            prefix="border"
            label="border color"
            value={borderColorToken}
            onChange={(v) => updateColor(/^border-[a-z]+-\d{2,3}$|^border-(black|white)$/, v)}
          />
          <ButtonGroupField
            label="border-w"
            ...
            onChange={(v) => updateToken(/^border(-[248])?$/, v)}
          />
          <ButtonGroupField
            label="radius"
            ...
            onChange={(v) => updateToken(/^rounded/, v)}
          />
          <ButtonGroupField
            label="font-size"
            ...
            value={fontSizeToken}
            placeholder="text-sm"
            onChange={(v) => updateToken(/^text-(xs|sm|base|lg|xl)$/, v)}
          />
          <ButtonGroupField
            label="font"
            ...
            onChange={(v) => updateToken(/^font-/, v)}
          />
          <ButtonGroupField
            label="text-align"
            options={["text-left", "text-center", "text-right"]}
            value={textAlignToken}
            placeholder="text-center"
            onChange={(v) => updateToken(/^text-(left|center|right)$/, v)}
          />
          <ButtonGroupField
            label="v-align"
            ...
            onChange={(v) => updateToken(/^items-/, v)}
          />
          <ButtonGroupField
            label="shadow"
...
export function ColumnDefsEditor({ value, selectedNodeId }: ColumnDefsEditorProps) {
  ...
  const [stylePanel, setStylePanel] = useState<{ col: number; btn: number } | null>(null);
  ...
  return (
    <>
      {stylePanel && (
        <StylePanel
          value={defs[stylePanel.col]?.actionButtons?.[stylePanel.btn]?.className}
          onChange={(v) => setActionBtnField(stylePanel.col, stylePanel.btn, "className", v)}
                ...
                {isActions && (
                  <div className="flex flex-col gap-1 pl-8">
                    ...
                    {(col.actionButtons ?? []).map((btn, bi) => (
                      ...
                      >
                        {/* Row 1: label input + 🎨 button + ✕ button */}
                        <div className="flex items-center gap-1">
                          <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                            <span className={LABEL_CLASS}>label</span>
                            <input
                          ...
                          <button
                            type="button"
                            onClick={() => setStylePanel({ col: idx, btn: bi })}
                            className={`${BTN_CLASS} mt-4 shrink-0`}
                            title="スタイル設定"
                          >
                            🎨
                          </button>
                        ...
                        {/* Row 2: className テキスト入力（直打ち用） */}
                        <div className="flex items-center gap-1 pl-1">
                          <span className={LABEL_CLASS}>class:</span>
                          <input
                            type="text"
                            value={btn.className ?? ""}
                            onChange={(e) => setActionBtnField(idx, bi, "className", e.target.value || undefined)}
                            placeholder="hover:bg-accent ..."
